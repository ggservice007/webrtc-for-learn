...
var createSignalingChannel = function(key, handlers) {
  ...
  function connect(failureCB) {
    ...
    function handler() {
      if(this.readyState == this.DONE) {
        if(this.status == 200 && this.response != null) {
          ...
          // then stat asnychronouse polling for messages
          id = res.id;
          status = res.status;
          poll();
    ...
    client.open("POST", "/connect?key=" +  key);
    client.send();
  }
  ...
  function poll() {
    ...
    (function getLoop() {
      get(function (response) {
        ...
        // if messages property exists, then we are conncted
  ...
  function get(getResponseHandler) {
    // reponse should either be error or a JSON object. If the
  ...
  function handleMessage(msg) {
    // process message asnychronously
